...
class ScipyBSplineCompression:
    ...
    def compress(
        ...
        for knots in generate_knots(t, data, s=s):
            #fit the trajecotry to get b-spline parameters
...
def chunk_bspline_trajectory(
    ...
    for start_idx in range(0, n_unique - 1, stride):
        ...
        if len(chunk_c) < expected_len: # len(chunk_c)一定小于期望值，本来就比knots少
    ...
    def _preprocess_chunks(
        ...
        for ep_idx in range(len(episode_ends)):
            ...
            for chunk in chunks:
                ...
                # the condition guarantees each sample's chunk domain starts at or after now 
