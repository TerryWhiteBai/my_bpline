...
# Date: June 8, 2026
#
# B-spline replay for the single YAM arm (drop-in companion to replay_episodes.py).
# Fits a smooth B-spline through the recorded action trajectory, then resamples it at
# a higher sample_freq and (optionally) sped up by --speed-up-times. The arm + gripper
# track the sped-up targets via YamArm.set_ik_dt_scale (which scales both the per-tick
# J-PARSE IK dt and the gripper slew-rate limit by the same factor).
...
YAM_TELEOP_DIR = Path(__file__).resolve().parents[3] / "simple_mobile" / "yam_teleop"
...
def main(args):
    ...
    # Create env (no live camera needed; images come from the saved episode)
    print("Creating environment...")
    env = RealEnv(use_cameras=False)  # Cameras add startup overhead; skip for replay
    print("Environment created")
    # Scale the per-tick J-PARSE IK dt on the YAM server so the arm can slew
    # fast enough to track sped-up TCP targets. Without this, the arm lags behind
    # the spline at speed_up_times > 1 and the executed path looks like a
    # low-pass-filtered, corner-cut version of the demo. The same scale also relaxes
    # the gripper slew-rate limit, so the gripper keeps pace instead of bottlenecking.
    env.arm.set_ik_dt_scale(args.speed_up_times)
    try:
