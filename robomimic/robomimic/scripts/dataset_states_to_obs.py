...
def get_camera_info(
    ...
    for cam_name in camera_names:
        ...
        if "eye_in_hand" in cam_name:
            ...
            # support robot0_eye_in_hand / robot1_eye_in_hand for bimanual envs
            robot_idx = int(cam_name.split("_")[0].replace("robot", ""))
            env_config = getattr(env.base_env, "env_configuration", None)
            if env_config == "bimanual":
                # Single Bimanual robot: robot_idx 0 -> right arm, 1 -> left arm
                robot = env.base_env.robots[0]
                arm_name = robot.arms[robot_idx]
                controller = robot.controller[arm_name]
            else:
                # Two independent robots
                robot = env.base_env.robots[robot_idx]
                controller = robot.controller
            eef_site_name = controller.eef_name
